...
function readFile(input) {
  $(".dropzone-wrapper").attr("style", "height: 296px");
  $('#msgError label').html("");

  if (input.files && input.files[0]) {
    console.log(input.files[0]);
    if (input.files[0].type !== "image/jpeg" && input.files[0].type !== "image/png") {
      $(".dropzone-wrapper").attr("style", "height: 296px; border: 2px dashed #dc3545 !important");
      $('#msgError label').html("png/jpg形式の画像を指定してください。");
      return;
    }
    if (input.files[0].size > 10*1024*1024) {
      $(".dropzone-wrapper").attr("style", "height: 296px; border: 2px dashed #dc3545 !important");
      $('#msgError label').html("10MB以下の画像を指定してください。");
      return;
    }
...
$(document).ready(function() {
  ...
  let initListUid = $('input#listUserID').val().split(",");
  $.ajax({
      type: 'GET',
      url: '/get-list-name',
      data: {
      ...
      $.ajax({
          ...
          success: function (response) {
              if ($('input#listUserID').val().length == 0) {
                  $('#body-push-noti').val($('#body-push-noti').val().replaceAll($('input#listUserID3').val(), nick));
                  $('input#listUserID2').val(response)
                  $('input#listUserID3').val(response)
              } else {
                  $('input#listUserID2').val(response)
                  if ($('input#listUserID3').val().length >0) {
                      $('#body-push-noti').val($('#body-push-noti').val().replaceAll($('input#listUserID3').val(), response));
                  }
                  if ($('#body-push-noti').val().includes(nick)) {
                      $('#body-push-noti').val($('#body-push-noti').val().replaceAll(nick, response));
                  }
                  $('input#listUserID3').val(response)
              }
          },
  ...
  $("#body-push-noti").on("blur", function() {
  ...
  $("#body-push-noti").on("focus", function() {
      const textChange = $('input#listUserID2').val();
      if (textChange.length > 0) {
          this.value = this.value.replaceAll(textChange, nick);
      }
  })
  $("#body-push-noti").on("change", function() {
